...
function backup {
  ...
  # date for directory and .tar file naming
  ...
  # Counting the number .tar files
  filenum=$(find "$dirto" -maxdepth 1 -name "*.tar*" -type f | wc -l)

  # Once 7 .tar are created, move them to a new week-ending directory
  ...
  # Create .tar file. Ideally this will work in a cron job, and you'll get daily backups
